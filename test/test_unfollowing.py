...
def test_ununfollowing_route():
    """
    GIVEN a Flask application configured for testing
    WHEN the '/unfollow (POST)' page is requested 
    ...
    with flask_app.test_client() as test_client:
        #Case 1 ==> To unfollow a user
        with check:
            response = test_client.patch('/unfollow', headers= headers, data=json.dumps(dict(username='uzair')))

            checking_response(response, response.data)
        
        #Case 2 ==> To unfollow with wrong username
        with check:
            response = test_client.patch('/unfollow', headers= headers, data=json.dumps(dict(username='qasim')))

            checking_response(response, response.data)

        #Case3 ==> null request
        with check:
            response = test_client.patch('/unfollow', headers= headers, data=json.dumps(dict(username='')))
